...
import { wellnessData, categories } from '@/lib/data';
...
export default function WellnessPage() {
  ...
  const categories_list = ['全部', '健身房', '瑜伽', '游泳', '养生', '球类'];
  const [adminData, setAdminData] = useState<AdminItem[]>([]);
  const [isClient, setIsClient] = useState(false);

  const loadData = () => {
    try {
      const items = getAllFromStorage('wellness');
      setAdminData(items);
      setIsClient(true);
    } catch (e) {
  ...
  const allData = [
    ...adminData.map((item) => ({
      id: item.id,
      name: item.data.name,
      category: item.data.category === '热门推荐' || item.data.category === '最新上线' ? '健身' : (item.data.category || '健身'),
      ...
      image: item.data.images && item.data.images.length > 0 ? item.data.images[0] : 'https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?w=800',
      images: item.data.images || [],
      videos: item.data.videos || [],
      description: item.data.description || '',
      isAdminAdded: true,
    })),
    ...wellnessData.map(item => ({ ...item, isAdminAdded: false, subCategory: '' })),
  ];

  return (
    <div className="min-h-screen bg-[#FFFDF7]">
      <div className="bg-gradient-to-r from-[#E91E63] to-[#C2185B] py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-wrap gap-2">
            {categories_list.map((cat) => (
              <button key={cat} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${cat === '全部' ? 'bg-[#E91E63] text-white' : 'bg-[#F5F5F5] text-[#636E72] hover:bg-[#E91E63]/10 hover:text-[#E91E63]'}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {isClient && adminData.length > 0 && (
          <div className="mb-4 p-3 bg-[#E91E63]/10 border border-[#E91E63]/30 rounded-xl">
            <span className="text-[#E91E63] text-sm">后台新增了 {adminData.length} 条内容</span>
          </div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {allData.map((item: any) => (
            <Link key={item.id} href={item.isAdminAdded ? `/wellness/admin-${item.id}` : `/wellness/${item.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
              <div className="relative h-56 overflow-hidden">
                <img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                <div className="absolute top-3 right-3 px-3 py-1 bg-[#E91E63] text-white text-xs rounded-full">{item.category}</div>
              ...
              <div className="p-5">
                <h3 className="font-bold text-lg text-[#2D3436] mb-2 group-hover:text-[#E91E63] transition-colors">{item.name}</h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#E8E8E8]">
                  <span className="text-[#E91E63] font-bold">{item.price}</span>
