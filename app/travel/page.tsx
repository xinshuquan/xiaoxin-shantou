...
import { travelData, categories } from '@/lib/data';
...
export default function TravelPage() {
  ...
  const categories_list = ['全部', '景点', '海岛', '博物馆', '公园', '历史遗迹'];
  const [adminData, setAdminData] = useState<AdminItem[]>([]);
  const [isClient, setIsClient] = useState(false);

  const loadData = () => {
    try {
      const items = getAllFromStorage('travel');
      setAdminData(items);
      setIsClient(true);
    } catch (e) {
  ...
  const allData = [
    ...adminData.map((item) => ({
      ...
      image: item.data.images && item.data.images.length > 0 ? item.data.images[0] : 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800',
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF7]">
      <div className="bg-gradient-to-r from-[#2ECC71] to-[#27AE60] py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-wrap gap-2">
            {categories_list.map((cat) => (
              <button key={cat} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${cat === '全部' ? 'bg-[#2ECC71] text-white' : 'bg-[#F5F5F5] text-[#636E72] hover:bg-[#2ECC71]/10 hover:text-[#2ECC71]'}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {isClient && adminData.length > 0 && (
          <div className="mb-4 p-3 bg-[#2ECC71]/10 border border-[#2ECC71]/30 rounded-xl">
            <span className="text-[#2ECC71] text-sm">后台新增了 {adminData.length} 条内容</span>
          </div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {allData.map((item: any) => (
            <Link key={item.id} href={item.isAdminAdded ? `/travel/admin-${item.id}` : `/travel/${item.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
              ...
              <div className="p-5">
                <h3 className="font-bold text-lg text-[#2D3436] mb-2 group-hover:text-[#2ECC71] transition-colors">{item.name}</h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#E8E8E8]">
                  <span className="text-[#2ECC71] font-bold">{item.price}</span>
