...
export default function NightlifePage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF7]">
      <div className="bg-gradient-to-r from-[#9B59B6] to-[#8E44AD] py-16">
            ...
            {categories_list.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  cat === '全部' ? 'bg-[#9B59B6] text-white' : 'bg-[#F5F5F5] text-[#636E72] hover:bg-[#9B59B6]/10 hover:text-[#9B59B6]'
                }`}
              >
                {cat}
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {nightlifeData.map((item) => (
            <Link
              key={item.id}
              href={`/nightlife/${item.id}`}
              className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300"
            >
              <div className="relative h-56 overflow-hidden">
                <img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                <div className="absolute top-3 right-3 px-3 py-1 bg-[#9B59B6] text-white text-xs rounded-full">{item.category}</div>
              </div>
              <div className="p-5">
                <h3 className="font-bold text-lg text-[#2D3436] mb-2 group-hover:text-[#9B59B6] transition-colors">{item.name}</h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#E8E8E8]">
                  <span className="text-[#9B59B6] font-bold">{item.price}</span>
