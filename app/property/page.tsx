...
export default function PropertyPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF7]">
      <div className="bg-gradient-to-r from-[#E67E22] to-[#D35400] py-16">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-wrap gap-2">
            {categories_list.map((cat) => (
              <button key={cat} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${cat === '全部' ? 'bg-[#E67E22] text-white' : 'bg-[#F5F5F5] text-[#636E72] hover:bg-[#E67E22]/10 hover:text-[#E67E22]'}`}>
                {cat}
              </button>
            ))}
          </div>
            ...
            <Link key={item.id} href={item.isAdminAdded ? `/property/admin-${item.id}` : `/property/${item.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#E8E8E8]">
                  <span className="text-[#E67E22] font-bold">{item.price}</span>
