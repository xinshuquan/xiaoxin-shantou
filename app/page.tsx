  ...
  return (
    <div className="bg-gradient-to-b from-[#0A0A0A] to-[#141414] min-h-screen">
      ...
      <section className="relative min-h-[600px] overflow-hidden">
        ...
        {/* Main Content */}
        <div className="relative max-w-7xl mx-auto px-4 h-full flex flex-col justify-center items-center text-center pt-20">
          {/* Logo */}
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#FFD700] to-[#D4AF37] flex items-center justify-center text-[#0D0D0D] text-5xl font-bold mb-8 animate-pulse-glow shadow-[0_0_50px_rgba(255,215,0,0.4)]">
            <span className="font-art">新</span>
          </div>
          
          <h1 className="text-5xl md:text-7xl font-bold text-gradient-gold mb-2 font-art glow-text">
            小新带你游汕头
          </h1>
          <p className="text-2xl md:text-3xl text-[#FFD700] mb-8 font-art animate-shimmer">
            吃喝玩乐不用愁！
          </p>
          <p className="text-lg text-[#8B7355] mb-10 max-w-2xl">
          ...
          <div className="w-full max-w-2xl relative mb-8">
            ...
            <button className="absolute right-2 top-1/2 -translate-y-1/2 px-8 h-12 bg-gradient-to-r from-[#FFD700] to-[#D4AF37] text-[#0D0D0D] rounded-full font-bold text-lg hover:shadow-[0_0_30px_rgba(255,215,0,0.5)] hover:scale-105 transition-all font-art">
              搜索
            </button>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['潮汕火锅', '海边民宿', '南澳岛', '家政服务', '招聘', '健身房'].map((tag, i) => (
              <Link
                key={tag}
                href={`/search?q=${tag}`}
                className="px-5 py-2 bg-[#1F1F1F]/60 backdrop-blur rounded-full text-sm text-[#C9A227] border border-[#3D3D3D] hover:bg-[#FFD700] hover:text-[#0D0D0D] hover:border-[#FFD700] transition-all hover:animate-wobble font-art"
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-[#FFD700] mb-3 font-art glow-text">全部分类</h2>
          <p className="text-[#8B7355] text-lg">发现汕头本地生活服务</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {categories.map((category, index) => (
            ...
            >
              <div
                className="w-16 h-16 rounded-2xl flex items-center justify-center text-4xl mb-4 mx-auto bg-[#1F1F1F] border border-[#3D3D3D] group-hover:border-[#FFD700] transition-all group-hover:animate-pulse"
              >
                {category.icon}
              </div>
              <h3 className="font-bold text-[#FFD700] text-center mb-1 font-art text-lg">{category.name}</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex justify-between items-center mb-12">
          <div>
            <h2 className="text-4xl font-bold text-[#FFD700] font-art glow-text">热门推荐</h2>
            <p className="text-[#8B7355] mt-2 text-lg">精选汕头本地优质商家</p>
          </div>
          <Link href="/food" className="text-[#FFD700] hover:underline font-art text-lg hover:animate-wobble">
            查看更多 →
          </Link>
        </div>
          ...
          {featuredData.map((item: any) => (
            ...
            >
              <div className="relative h-52 overflow-hidden">
                ...
                />
                <div className="absolute top-3 left-3 px-4 py-1.5 bg-gradient-to-r from-[#FFD700] to-[#D4AF37] text-[#0D0D0D] text-xs font-bold rounded-full font-art">
                  {item.categoryName}
                </div>
              </div>
              <div className="p-5">
                <h3 className="font-bold text-lg text-[#FFD700] mb-2 font-art">{item.name}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-r from-[#FFD700] to-[#D4AF37] flex items-center justify-center text-3xl animate-pulse-glow">🍜</div>
              <div>
                <h2 className="text-4xl font-bold text-[#FFD700] font-art glow-text">美食推荐</h2>
                <p className="text-[#8B7355] mt-1 text-lg">潮汕美食，等你来尝</p>
              </div>
            </div>
            <Link href="/food" className="text-[#FFD700] hover:underline font-art text-lg">
              查看全部 →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {foodData.slice(0, 6).map((item) => (
              ...
              >
                ...
                <div className="p-3">
                  <h3 className="font-medium text-sm text-[#FFD700] line-clamp-1 font-art">{item.name}</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-[#0D0D0D]">
            {[
              ...
              <div key={i} className="p-6 bg-[#0D0D0D]/10 rounded-2xl backdrop-blur hover:scale-105 transition-transform">
                ...
                <div className="text-4xl font-bold mb-2 font-art">{stat.num}</div>
                <div className="text-[#0D0D0D]/80 font-art text-lg">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex justify-between items-center mb-12">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-[#1F1F1F] border-2 border-[#FFD700] flex items-center justify-center text-3xl">🗺️</div>
            <div>
              <h2 className="text-4xl font-bold text-[#FFD700] font-art glow-text">周边旅游</h2>
              <p className="text-[#8B7355] mt-1 text-lg">发现汕头的美丽风景</p>
            </div>
          </div>
          <Link href="/travel" className="text-[#FFD700] hover:underline font-art text-lg">
            查看全部 →
          </Link>
        </div>
          ...
          {travelData.map((item) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <h3 className="font-bold text-xl text-[#FFD700] mb-2 font-art">{item.name}</h3>
                <div className="flex items-center justify-between">
      ...
      <section className="py-20 bg-[#141414] border-t border-[#2D2D2D]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-4xl font-bold text-[#FFD700] mb-6 font-art glow-text">成为商家，加入我们</h2>
          <p className="text-[#8B7355] mb-8 max-w-2xl mx-auto text-lg">
            入驻「小新带你游汕头」，让更多汕头人和游客发现您的店铺
          </p>
          <Link
            href="/merchant"
            className="inline-flex items-center gap-3 px-10 py-5 bg-gradient-to-r from-[#FFD700] to-[#D4AF37] text-[#0D0D0D] rounded-full font-bold text-xl hover:shadow-[0_0_40px_rgba(255,215,0,0.6)] hover:scale-105 transition-all font-art"
          >
